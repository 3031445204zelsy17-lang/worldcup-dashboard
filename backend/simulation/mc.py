...
- 淘汰赛: 逐轮向量化(R32→R16→QF→SF→Final), 每轮构造 per-sim 对阵 → 批量 Poisson 采样 →
          胜者(平局 50/50 近似点球). 场地 neutral 用 format.venue_neutral.
    ...
    def _simulate_knockout(self, n: int, winners, runners, third, third_pts, third_gd, third_gf):
        ...
        for sim in range(n):
            ...
            for round_name, matches in fmt.ROUNDS:   # ro32/ro16/qf/sf
                ...
                # 胜者(平局 50/50 近似点球)
                coin = self.rng.random(len(matches)) < 0.5
                win_idx = np.where(hg > ag, home_idx, np.where(hg == ag, np.where(coin, home_idx, away_idx), away_idx))
            ...
            if fhg == fag:
                champion = h if self.rng.random() < 0.5 else a
            else:
                champion = h if fhg > fag else a
            finalist = (h, a)
